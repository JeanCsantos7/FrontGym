...
const DetalhesExercicios = () => {
 ...
 const fetchAPI = async () => {
  ...
  const adicionarExercicio = async (e: React.FormEvent<HTMLFormElement>, item: exerciciosTypes) => {
  ...
  return (
    <>
      {result.map((item) => (
        ...
        <form  method="POST" onSubmit={(e) => adicionarExercicio(e,item)}>
        <div
          key={item.id}
          className="flex flex-col items-center px-6 py-8 max-w-xl mx-auto"
        >
